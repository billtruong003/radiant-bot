...
- **2026-06-01** (Phase 14.9): **`/selftest` admin-only diagnostic ships.** 7 read-only checks run in parallel (`Promise.all`): Store counts + snapshot age, Scheduler task count (≥6 expected), Discord guild reachability + member/channel/ping, `/health` HTTP fetch, Backup freshness (`backup-repo/snapshot.json` mtime — >36h = fail = nightly broken), Aki 24h budget vs `AKI_DAILY_BUDGET_USD`, Automod 24h hit count. Each check wrapped in `safeRun()` so one exception doesn't abort the rest. Output composed as a themed embed (success/warn/danger by worst status) + attached `selftest-log.json` if any non-OK. Result DM'd to caller; ephemeral reply only confirms. Falls back to ephemeral content if DM blocked. Targets <3s wall-clock. Added `getSchedulerTaskCount()` getter to `modules/scheduler/index.ts` (private `tasks` array exposure). Use case: after deploy, run `/selftest` once and see in one screenshot which subsystem is sad. Bill: backup-repo PAT expired ~2026-05-13 → fail check would catch this in future.
- **2026-06-02** (Phase 14.10): **`/inventory` + `/shop` equip/buy menus paginated.** Bill 31 weapons → only 25 visible in dropdown — items 26-31 (incl. Bản Mệnh + Tiên Phẩm) unreachable. Root cause: every builder did `.slice(0, 25)` to obey Discord's StringSelectMenu hard cap. Fix: extracted `src/utils/pagination.ts` (`pageOf<T>()` + `buildPageNavRow()`); builders now return ALL options; central `renderActiveTab()` (inventory) / `buildComponents()` (shop) paginates + appends 3-button nav row `[◀ · Trang N/M · ▶]` when `totalPages > 1`. Page state lives in closure per-invocation `Record<Kind, number>`. Tab switch resets page to 0; equip/buy preserves page (so users equipping page-2 items stay on page 2 after refresh). `pageOf` clamps OOB indices so post-purchase shrinkage doesn't strand users on an empty page. CustomId scheme: `inv:page:<kind>:<dir>:<userId>` / `shop:page:<tab>:<dir>:<userId>`. Affected: 4 inventory equip menus + 1 shop buy menu (5 fixes). 505/505 tests still green.
