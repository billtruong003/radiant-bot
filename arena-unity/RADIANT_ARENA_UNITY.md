...
# Manifest: Packages/manifest.json
{
  "dependencies": {
    ...
    // UI Toolkit is built-in (`com.unity.modules.uielements`) in Unity 6 — không cần add package.
    // DEPRECATED ref to "com.unity.ui": "1.x" removed (legacy Unity 2020 path).
...
> **⚠️ Fix 2026-05-15**: BillGameCore tự boot qua `[RuntimeInitializeOnLoadMethod]` (xem `Assets/BillGameCore/Runtime/Bootstrap/Bill.cs`). **KHÔNG có `Bill.Init()` method**. Gate ArenaBootstrap logic bằng `Bill.IsReady` hoặc `Bill.Events.SubscribeOnce<GameReadyEvent>`. Framework cần `Assets/Resources/BillBootstrapConfig.asset` để boot — verify exists trước D.U1.
...
public class ArenaBootstrap : MonoBehaviour {
    ...
    void Start() {
        // BillGameCore auto-boots BeforeSceneLoad → AfterSceneLoad. Khi Start() chạy,
        // Bill.IsReady thường đã true. Gate phòng race với scene load order.
        if (Bill.IsReady) InitArena();
        else Bill.Events.SubscribeOnce<GameReadyEvent>(_ => InitArena());
    }

    void InitArena() {
        // 1. Register Arena-specific states (BillGameCore có sẵn BootState/MenuState/GameplayState/etc — đây là Arena layer riêng)
    ...
    // Helper bỏ qua — không cần wrap async khi đã có SubscribeOnce pattern ở Start().
